...
class ImportApiAccountScreen extends ConsumerStatefulWidget {
  const ImportApiAccountScreen({super.key});

  @override
  ConsumerState<ImportApiAccountScreen> createState() =>
      _ImportApiAccountScreenState();
}

class _ImportApiAccountScreenState
    extends ConsumerState<ImportApiAccountScreen> {
  ...
  Future<void> _onSubmit() async {
    ...
    try {
      ...
      LoggingService.instance.debug(
        'API account imported successfully',
        tag: 'IMPORT_API_ACCOUNT',
      );
      ...
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text(l10n.importApiAccountRegistrationFailed(e.toString()))),
      );
    } finally {
