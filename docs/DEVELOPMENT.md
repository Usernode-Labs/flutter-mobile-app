...
This guide provides conventions, paths, and best practices for developers working in this repository.
...
Uses `flutter_rust_bridge.yaml` configuration.
...
Supply `--dart-define` or `--dart-define-from-file=.env` as needed.
...
CI runs format/analyze/tests on PRs.

## Common Flags

- **Result-based node providers**: `--dart-define=USE_RESULT_PROVIDERS=true`
- **Feature toggles**: `--dart-define=ENABLED_FEATURES=home,wallet,dapps,profile` or `all`
- **Disable specific tags**: `--dart-define=DISABLED_FEATURES=wallet.bridge`
- **Sentry DSN**: `--dart-define=SENTRY_DSN=...` (omit to disable Sentry)
- **GitHub feedback**: `--dart-define=GITHUB_TOKEN=ghp_xxx` (required for `GitHubIssueService`)

## Feature Flags

- Top-level features in `FeatureFlags.ordered`. Check with `FeatureFlags.isEnabled(AppFeature.wallet)`
- Granular keys (tags) via `FeatureFlags.on('wallet.send')` with default-on behavior unless disabled
- Can also configure `assets/feature_flags.json` with keys `enabled`, `disabled`, and `order`

## Localization

- Edit ARB files under `lib/core/l10n/` (e.g., `app_en.arb`)
- Run `flutter gen-l10n` after changes. Generated code lives alongside ARB files
- Use `AppLocalizations.of(context)` in UI; avoid hard-coded strings in widgets

## Navigation

- Uses `go_router` with a shell route
- Add/adjust routes in `lib/core/routing/app_router.dart`
...
- Provider naming: end with `Provider`; async variants expose `AsyncValue<T>` to UI
...
## Developer Checklist (Before You Finish)
...
- [BACKGROUND_PRODUCTION.md](./BACKGROUND_PRODUCTION.md) - Background block production system
