...
| **Battery %** | `battery_level` | ✅ | ✅ | Integer 0-100 or `null`. Returns `null` when battery info is unavailable (distinguishes "unavailable" from "0%"). Uses `battery_plus` plugin (cross-platform). Returns `0` on simulators/emulators. Collected fresh each time (no caching). Source: `metrics_collector_service.dart:324-330`. |
...
| **Exact Alarms** | `permission_exact_alarms` (fallback: `exact_alarms_permission`) | ⚠️ | ✅ | Boolean indicating if app can schedule exact alarms. **iOS**: Concept doesn't exist; uses `notification_permission` as fallback/proxy. **Android**: Always `true` with SET_ALARM_CLOCK API (no permission required). Previously used `AlarmManager.canScheduleExactAlarms()` but migrated to alarm clock API for Google Play compliance. |
