...
void main() {
  testWidgets('WalletScreen shows UTXO error text', (tester) async {
    final container = ProviderContainer(overrides: [
      walletProvider.overrideWith(() => _OkWalletController()),
      walletUtxosProvider.overrideWith(() => _ErrorUtxosController()),
    ]);
    ...
    expect(find.text('Failed to load UTXOs'), findsOneWidget);
  });
}

class _OkWalletController extends WalletController {
  @override
...
class _ErrorUtxosController extends WalletUtxosController {
  @override
  Future<List<OwnedUtxo>> build() async =>
      throw const BackendError('Failed to load UTXOs');
}
